...
A free and open-source module to edit the Wi-Fi MAC address on Android devices by hooking the system Wi-Fi service. It can force enable MAC randomization support exposed by Android. You can use it, for example, to customize MAC behavior for privacy, or to assist devices with limited captive portal support to access the Internet on certain Wi-Fi networks.

## Compatibility

- Android 10+ (tested up to Android 16 QPR2)
...
This is useful on devices where the hardware and chipset drivers support MAC randomization but the device vendor does not implement proper software support. This can also happen on some alternative Android builds where MAC randomization is not explicitly enabled. On certain Qualcomm devices, hardware support can be checked by looking at `/vendor{/etc/wifi/kiwi_v2,firmware/wlan/qca_cld}/WCNSS_qcom_cfg.ini`. For legacy Qualcomm devices without MAC randomization support, you can alternatively consider editing `wlan_mac.bin` or `/sys/wifi/mac_addr` directly instead of using this module.

## Credits

- [David Berdik](https://f-droid.org/repo/com.berdik.macsposed_6_src.tar.gz) for the initial open-source implementation of system server hook implementation